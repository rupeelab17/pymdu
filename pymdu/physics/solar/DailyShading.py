...
from pymdu.physics.solar.UtilitiesSolar import (
    saveraster,
    sun_position,
...
def dailyshading(
    ...
    print('ITERATION timeInterval', timeInterval)
    ...
    for i in range(0, itera):
        ...
        shadowresult['time_vector'] = time_vector
        ...
        if alt[i] > 0:
            if wallshadow == 1:  # Include wall shadows (Issue #121)
                if usevegdem == 1:
                    ...
                    sh = sh - (1 - vegsh) * (1 - psi)
                    if onetime == 0:
                        filenamewallshve = os.path.join(
                            folder,
                            'Facadeshadow_fromvegetation_' + timestr + '_LST.tif',
                        )
                ...
                if onetime == 0:
                    ...
                    filenamewallsh = os.path.join(
                        folder, 'Facadeshadow_frombuilding_' + timestr + '_LST.tif'
                    )
                    saveraster(gdal_data, filenamewallsh, wallsh)
            else:
                if usevegdem == 0:
                    sh = shadowingfunctionglobalradiation(dsm, azi[i], alt[i], scale, 0)
                    # shtot = shtot + sh
                else:
                    shadowresult = shadowingfunction_20(
                        dsm, vegdem, vegdem2, azi[i], alt[i], scale, amaxvalue, bush
                    )
                ...
                if onetime == 0:
                    filename = os.path.join(folder, 'Shadow_' + timestr + '_LST.tif')
                    saveraster(gdal_data, filename, sh)
