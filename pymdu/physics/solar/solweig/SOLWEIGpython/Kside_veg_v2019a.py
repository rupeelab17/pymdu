...
import numpy as np
from pymdu.physics.solar.solweig.SOLWEIGpython.Kvikt_veg import Kvikt_veg


def Kside_veg_v2019a(
    radI,
    radD,
    radG,
    shadow,
    ...
    if ani == 1:
        ...
        if cyl == 1:
            for idx in range(0, 145):
                anglIncC = (
                    np.cos(aniAlt[idx] * deg2rad) * np.cos(0) * np.sin(np.pi / 2)
                    + np.sin(aniAlt[idx] * deg2rad) * np.cos(np.pi / 2)
