...
def psychrometrics(Tdb_in, w_in, P):
    ...
    """
    # Change units
    c_air = 1006.0  # [J/kg] air heat capacity, value from ASHRAE Fundamentals
    hlg = 2501000.0  # [J/kg] latent heat, value from ASHRAE Fundamentals
    cw = 1860.0  # [J/kg] value from ASHRAE Fundamentals
    ...
    Tdp = (
        ...
        + pow(alpha, 2) * 0.7389
        + pow(alpha, 3) * 0.09486
        + pow(_pw, 0.1984) * 0.4569
    )  # valid for Tdp between 0 C and 93 C
...
def saturation_pressure(Tdb_):
    ...
    return _Pws / 1000.0  # kPa


def moist_air_density(P, Tdb, H):
    """Moist air density (kgv m-3) from dry bulb example, HR, and pressure[1][2][3].
    ...
    return P / (1000 * 0.287042 * Tdb * (1.0 + 1.607858 * H))
