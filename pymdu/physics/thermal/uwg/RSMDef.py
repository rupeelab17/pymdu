...
class RSMDef(object):
    ...
    def __init__(self, lat, lon, gmt, height, T_init, P_init, parameter, z_meso_path):
        ...
        for iz in range(1, self.nzref):
            self.presProf[iz] = (
                self.presProf[iz - 1] ** (parameter.r / parameter.cp)
                - parameter.g
                / parameter.cp
                * (P_init ** (parameter.r / parameter.cp))
                * (1.0 / self.tempProf[iz] + 1.0 / self.tempProf[iz - 1])
                * 0.5
                * self.dz[iz]
            ) ** (1.0 / (parameter.r / parameter.cp))

        self.tempRealProf = [T_init for x in range(self.nzref)]
        for iz in range(self.nzref):
            self.tempRealProf[iz] = self.tempProf[iz] * (
                self.presProf[iz] / P_init
        ...
        for iz in range(1, self.nzref):
            self.densityProfS[iz] = (
                self.densityProfC[iz] * self.dz[iz - 1]
                + self.densityProfC[iz - 1] * self.dz[iz]
            ) / (self.dz[iz - 1] + self.dz[iz])
    ...
    def vdm(self, forc, rural, parameter, simTime):
        ...
        for iz in range(self.nzfor):
            self.ublPres = self.ublPres + self.presProf[iz] * self.dz[iz] / (
                self.z[self.nzref - 1] + self.dz[self.nzref - 1] / 2.0
            )

    def diffusion_coefficient(
        self, rho, z, dz, z0, disp, tempRur, heatRur, nz, uref, th, parameter
    ):
        # Initialization
        ...
        ustar = parameter.vk * uref / log((10.0 - disp) / z0)
        ...
        if heatRur > 1e-2:
            ...
            wstar = (
                parameter.g
                * heatRur
                * parameter.dayBLHeight
                / rho
                / parameter.cp
                / tempRur
            ) ** (1 / 3.0)
            # Wind profile function
            phi_m = (1 - 8.0 * 0.1 * parameter.dayBLHeight / lengthRur) ** (-1.0 / 3.0)

            for iz in range(nz):
                # Mixed-layer velocity scale
                ws[iz] = (
                    ustar**3
                    + phi_m * parameter.vk * wstar**3 * z[iz] / parameter.dayBLHeight
                ) ** (1 / 3.0)

                # TKE approximation
                te[iz] = max(ws[iz] ** 2.0, 0.01)

        else:
            # Stable and neutral conditions
            for iz in range(nz):
                # TKE approximation
                te[iz] = max(ustar**2.0, 0.01)
    ...
    def diffusion_equation(nz, dt, co, da, daz, cd, dz):
        ...
        for iz in range(1, nz):
            cddz[iz] = 2.0 * daz[iz] * cd[iz] / (dz[iz] + dz[iz - 1])
    ...
    def dissipation_bougeault(g, nz, z, dz, te, pt):
        ...
        for iz in range(nz):
            zup = 0.0
            dlu[iz] = z[nz] - z[iz] - (dz[iz] / 2.0)
            zzz = 0.0
            zup_inf = 0.0
            beta = g / pt[iz]

            for izz in range(iz, nz - 1):
                dzt = (dz[izz + 1] + dz[izz]) / 2.0
                zup = zup - beta * pt[iz] * dzt
                zup = zup + beta * (pt[izz + 1] + pt[izz]) * dzt / 2.0
                zzz = zzz + dzt
                ...
                if (te[iz] < zup) and ((te[iz] > zup_inf) or _chkzero):
                    ...
                    if not is_near_zero(bbb - 0.0):
                        _t1 = sqrt(
                            max(
                                0.0,
                                (beta * (pt[izz] - pt[iz])) ** 2.0
                                + 2.0 * bbb * beta * (te[iz] - zup_inf),
                            )
                        )
                        tl = (-beta * (pt[izz] - pt[iz]) + _t1) / bbb / beta
                    else:
                        tl = (te[iz] - zup_inf) / (beta * (pt[izz] - pt[iz]))
                    dlu[iz] = max(1.0, zzz - dzt + tl)
                zup_inf = zup

            zdo = 0.0
            zdo_sup = 0.0
            dld[iz] = z[iz] + dz[iz] / 2.0
            zzz = 0.0

            for izz in range(iz, 0, -1):
                dzt = (dz[izz - 1] + dz[izz]) / 2.0
                zdo = zdo + beta * pt[iz] * dzt
                zdo = zdo - beta * (pt[izz - 1] + pt[izz]) * dzt / 2.0
                zzz = zzz + dzt
    ...
    def load_z_meso(z_meso_path):
        ...
        assert os.path.exists(z_meso_path), "File: '{}' does not exist.".format(
            z_meso_path
        )
    ...
    def __repr__(self):
        return (
            'RSM,\n lat: {}\n lon: {}\n gmt: {}\n height: {}\n z0r: {}\n '
            'disp: {}'.format(
                self.lat, self.lon, self.gmt, self.height, self.z0r, self.disp
            )
        )
