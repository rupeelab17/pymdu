...
class UWG(object):
    ...
    CP = 1004.0  # heat capacity for air (J/kg K)
    VK = 0.40  # von karman constant (dimensionless)
    R = 287.0  # gas constant dry air (J/kg K)
    ...
    WATERDENS = 1000.0  # water density (kg m-3)
    LVTT = 2.5008e6  #
    TT = 273.16  #
    ESTT = 611.14  #
    CL = 4.218e3  #
    ...
    COLBURN = math.pow((0.713 / 0.621), (2 / 3.0))
    ...
    @classmethod
    def from_param_file(
        cls, param_path, epw_path=None, new_epw_dir=None, new_epw_name=None
    ):
        ...
        assert os.path.exists(param_path), (
            'Parameter file "{}" does not ' 'exist.'.format(param_path)
        )

        assert param_path.endswith('.uwg'), (
            'Parameter file must be a ".uwg" ' 'filetype. Got: {}.'.format(param_path)
        )
    ...
    def from_param_args(
        ...
        assert refcheck != 1, (
            'The ref_sch_vector and ref_bem_vector must both be '
        ...
        if refcheck == 2:
            model.ref_bem_vector, model.ref_sch_vector = model._check_reference_data(
    ...
    def from_dict(cls, data, epw_path=None, new_epw_dir=None, new_epw_name=None):
        ...
        """
        assert data['type'] == 'UWG', 'Expected UWG dictionary. Got {}.'.format(
            data['type']
        )
        ...
        assert refcheck != 1, (
            'The ref_sch_vector and ref_bem_vector '
        ...
        if refcheck == 2:
            ref_sch_vector = [
                SchDef.from_dict(schdict) for schdict in data['ref_sch_vector']
            ]
            ref_bem_vector = [
                BEMDef.from_dict(bemdict) for bemdict in data['ref_bem_vector']
            ]
    ...
    def epw_path(self, value):
        if value is not None:
            assert os.path.exists(value), 'File: "{}" does not exist.'.format(value)
    ...
    def bld(self, value):
        assert isinstance(value, (list, tuple)), (
            'The bld property must be a list. ' 'Got {}.'.format(value)
        )

        # check values and fraction sum.
        total_frac = 0.0
        for bld_row in value:
            assert len(bld_row) == 3, (
                'Each bld tuple must contain three '
            ...
            assert isinstance(builtera, str) and builtera.lower() in REF_BUILTERA_SET, (
                ...
                'era as one of {}. Got: {}.'.format(REF_BUILTERA, builtera.lower())
            )
            assert 0.0 <= frac <= 1.0, (
                'The third item in the bld tuple '
                'must be a value between 0 and 1, inclusive, defining the '
                'fraction of total built stock. Got: {}.'.format(frac)
            )
            total_frac += frac

        assert abs(total_frac - 1.0) < 1e-2, (
            'The sum of reference building '
            'fractions defined in bld must equal one. Got: {}.'.format(total_frac)
        )
    ...
    @zone.setter
    def zone(self, value):
        assert isinstance(value, str), 'zone must be a string. Got: {}.'.format(value)
        value = value.upper()
    ...
    def blddensity(self, value):
        try:
            assert self.vegcover + value <= 1, (
                'The sum of the blddensity, treecover '
    ...
    def treecover(self, value):
        try:
            assert self.grasscover + self.blddensity + value <= 1, (
                'The sum of the '
    ...
    def grasscover(self, value):
        try:
            assert self.treecover + self.blddensity + value <= 1, (
                'The sum of the '
                'blddensity, treecover and grasscover ratios must be less than one. '
                'Got: {}, {} and {}, respectively.'.format(
    ...
    def simulate(self):
        ...
        * USMData - N x 1 output vector of USM object instance
        ...
        self.ph = self.simTime.dt / 3600.0  # dt (simulation time step) in hours
        ...
        for it in tqdm(range(1, self.simTime.nt, 1)):
            if it % day_step == 0:
                pass
            ...
            if self.nSoil < 3:
                ...
                self.forc.deepTemp = self.Tsoil[self._soilindex1][
                    self.simTime.month - 1
                ]
            ...
            # wind speed (m s-1)
            self.forc.wind = max(
                self.forcIP.wind[self.ceil_time_step], self.geoParam.windMin
            )
            # wind direction
            ...
            elif utilities.is_near_zero(self.simTime.julian % 7 - 6.0, 1e-10):
            ...
            for i in range(len(self.BEM)):
                ...
                self.BEM[i].building.cool_setpoint_night = self.BEM[
                    i
                ].building.cool_setpoint_day
                # add from temperature schedule for heating
                self.BEM[i].building.heat_setpoint_day = (
                    self.Sch[i].heat[di][hi] + 273.15
                )
                self.BEM[i].building.heat_setpoint_night = self.BEM[
                    i
                ].building.heat_setpoint_day
            ...
            self.rural.infra = (
                self.forc.infra
                - self.rural.emissivity * self.SIGMA * self.rural.layerTemp[0] ** 4.0
            )  # Infrared radiation from rural road
            self.rural.SurfFlux(
                self.forc,
                self.geoParam,
                self.simTime,
            ...
            # Uforc.example = copy.copy(self.UCM.canTemp)
    ...
    def write_epw(self):
        ...
        for iJ in range(len(self.UCMData)):
            ...
            self.epwinput[iJ + self.simTime.timeInitial - 8][6] = '{0:.{1}f}'.format(
                self.UCMData[iJ].canTemp - 273.15, epw_prec
            )
            # dew point temperature [?C]
            self.epwinput[iJ + self.simTime.timeInitial - 8][7] = '{0:.{1}f}'.format(
                self.UCMData[iJ].Tdp, epw_prec
            )
            # relative humidity     [%]
            self.epwinput[iJ + self.simTime.timeInitial - 8][8] = '{0:.{1}f}'.format(
                self.UCMData[iJ].canRHum, epw_prec
            )
            # wind speed [m/s]
            self.epwinput[iJ + self.simTime.timeInitial - 8][21] = '{0:.{1}f}'.format(
                self.WeatherData[iJ].wind, epw_prec
            )
        ...
        print('New climate file is generated at {}.'.format(self.new_epw_path))
    ...
    def _read_input(self, param_path):
        ...
        while count < len(param_data):
            ...
            try:
                if row == [] or '#' in row[0]:
                    ...
                    count += 1
                    bld = []
                    nextrow = [c.replace(' ', '').lower() for c in param_data[count]]
                    while len(nextrow) > 0 and nextrow[0] in REF_BLDTYPE_SET:
                        ...
                        count += 1
                        nextrow = [
                            c.replace(' ', '').lower() for c in param_data[count]
                        ]
                ...
                print(e)
                print(
                    'Error while reading parameter at row {}. Got: {}.'.format(
                        count, row
                    )
                )
        ...
        # Set UWG parameters
        for attr in self.PARAMETER_LIST:
            assert attr in self._init_param_dict, (
                'The {} attribute is not defined in ' 'the {} parameter file.'.format(
    ...
    def _compute_BEM(self):
        ...
        * Sch - list of Schedule objects extracted from readDOE
        ...
        k = 0
        self.r_glaze_total = 0.0
        self.SHGC_total = 0.0
        self.alb_wall_total = 0.0
        h_floor = self.flr_h or 3.05  # average floor height

        total_urban_bld_area = (
            math.pow(self.charlength, 2) * self.blddensity * self.bldheight / h_floor
        ...
        bld_dict = {
            bldg[0] + bldg[1]: (bldg[0], REF_BUILTERA.index(bldg[1]), bldg[2])
        ...
        for i in range(len(self.refBEM)):  # ~16 building types (more w/ custom refs)
            for j in range(3):  # ~ 3 built eras
                ...
                ref_key = self.refBEM[i][j][0].bldtype + self.refBEM[i][j][0].builtera
                if ref_key in bld_dict:
                    ...
                    self.Sch.append(self.refSchedule[i][builtera_idx][zone_idx])
                    k += 1
    ...
    def _compute_input(self):
        ...
        * rural - Element object for rural road
        """
        self.simTime = SimParam(
            self.dtsim, self.dtweather, self.month, self.day, self.nday
        ...
        nightStart = 18.0  # arbitrary values for begin/end hour for night setpoint
        nightEnd = 8.0
        maxdx = 250.0  # max dx (m)
        ...
        self.road = Element(
            self.road.albedo,
            self.road.emissivity,
            newthickness,
            roadMat,
            self.road.vegcoverage,
            self.road.layerTemp[0],
        ...
        self.rural = Element(
            self.rural.albedo,
            self.rural.emissivity,
            newthickness,
            ruralMat,
            self.rural.vegcoverage,
            self.rural.layerTemp[0],
            self.rural.horizontal,
            self.rural.name,
        )
    ...
    def _hvac_autosize(self):
        ...
        for i in range(len(self.BEM)):
            if self.autosize:
                self.BEM[i].building.coolcap = 9999.0
                self.BEM[i].building.heat_cap = 9999.0
    ...
    def _check_reference_data(self, ref_bem_vector, ref_sch_vector):
        ...
        assert len(ref_sch_vector) == len(ref_bem_vector), (
            'The '
        ...
        assert all(isinstance(v, SchDef) for v in ref_sch_vector), (
            'All items in the ' 'ref_sch_vector must be a SchDef object.'
        )
        assert all(isinstance(v, BEMDef) for v in ref_bem_vector), (
            'All items in the ' 'ref_bem_vector must be a BEMDef object.'
        )

        for ref_bem, ref_sch in zip(ref_bem_vector, ref_sch_vector):
            assert ref_bem.bldtype == ref_sch.bldtype, (
                'The bldtype for '
                'corresponding items in ref_bem_vector and ref_sch_vector must be the '
                'same. Got {} and {}.'.format(ref_bem.bldtype, ref_sch.bldtype)
            )
            assert ref_bem.builtera == ref_sch.builtera, (
                'The builtera for '
                'corresponding items in ref_bem_vector and ref_sch_vector must be the '
                'same. Got {} and {}.'.format(ref_bem.builtera, ref_sch.builtera)
            )
    ...
    def _customize_reference_data(self):
        ...
        for sch in self.ref_sch_vector:
            ...
            try:
                ...
                print(
                    'Overwrite DOE reference schedule "{} {}" '
                    'with custom schedule.'.format(sch.builtera, sch.bldtype)
                )
            except ValueError:
                # Add new rows based on type index in object
                ti = len(self.refSchedule)
                self.refSchedule.append([[None for c in range(16)] for r in range(3)])
                print(
                    'Add custom schedule for "{} {}".'.format(sch.builtera, sch.bldtype)
                )
        ...
        for bem in self.ref_bem_vector:
            ...
            try:
                ...
                print(
                    'Overwrite DOE reference BEM "{} {}" '
                    'with custom schedule.'.format(bem.builtera, bem.bldtype)
                )
            except ValueError:
                # Add new rows based on type index in object
                ti = len(self.refBEM)
                self.refBEM.append([[None for c in range(16)] for r in range(3)])
                print('Add custom bem for "{} {}".'.format(bem.builtera, bem.bldtype))
    ...
    def load_refDOE(refDOE_path=REFDOE_PATH):
        ...
        Returns:
            ...
            respectively.
        """
        assert os.path.exists(refDOE_path), 'File: {} does not exist.'.format(
            refDOE_path
        )
        ...
        if len(materials.layer_thickness_lst) > 1:
            for j in range(len(materials.layer_thickness_lst)):
                ...
                # Material that's less then min_thickness is not added.
                    ...
                    print(
                        'WARNING: Material layer too thin (less then 2 cm) to process.'
            ...
            # should give warning and stop. Only warning given for now.
                ...
                newthickness = [min_thickness / 2.0, min_thickness / 2.0]
                newmat = [
                    Material(k[0], Vhc[0], name=materials.name),
                    Material(k[0], Vhc[0], name=materials.name),
                ]
                print(
                    'WARNING: Material layer less then 2 cm is found.'
                ...
                newthickness = [
                    materials.layer_thickness_lst[0] / 2.0,
                    materials.layer_thickness_lst[0] / 2.0,
                ]
                newmat = [
                    Material(k[0], Vhc[0], name=materials.name),
