...
class Element(object):
    ...
    def __init__(
        ...
        assert len(layer_thickness_lst) == len(material_lst), (
            'The number of layer '
    ...
    def from_dict(cls, data):
        ...
        assert data['type'] == 'Element', (
            'Expected ' 'Element dictionary. Got {}.'.format(data['type'])
        )
    ...
    def SurfFlux(
        self, forc, parameter, simTime, humRef, tempRef, windRef, boundCond, intFlux
    ):
        ...
        dens = forc.pres / (1000 * 0.287042 * tempRef * (1.0 + 1.607858 * humRef))
        ...
        if self.horizontal:
            ...
            if not is_near_zero(self.waterStorage) and self.waterStorage > 0.0:
                ...
                self.waterStorage = min(
                    self.waterStorage + simTime.dt * (forc.prec - eg), parameter.wgmax
                )
                self.waterStorage = max(self.waterStorage, 0.0)  # [m]
            else:
                eg = 0.0
            ...
            if simTime.month < parameter.vegStart and simTime.month > parameter.vegEnd:
                ...
                # Summer, veg
                self.solAbs = (
                    (1.0 - self.vegcoverage) * (1.0 - self.albedo)
                ...
                try:
                    ...
                    vegSen = (
                        self.grasscoverage
                        * (1.0 - parameter.vegAlbedo)
                        * (1.0 - parameter.grassFLat)
                        * self.solRec
                    )
                    vegSen += (
                        self.treecoverage
                        * (1.0 - parameter.vegAlbedo)
                        * (1.0 - parameter.treeFLat)
                        * self.solRec
                    )
        ...
        self.layerTemp = self.Conduction(
            simTime.dt, self.flux, boundCond, forc.deepTemp, intFlux
        )
    ...
    def Conduction(self, dt, flx1, bc, temp2, flx2):
        ...
        for j in range(1, num):
            tcp[j] = 2.0 / (d[j - 1] / tc[j - 1] + d[j] / tc[j])
            hcp[j] = hc[j] * d[j]
        ...
        if is_near_zero(bc - 1.0):
            ...
            za[num - 1][2] = 0.0
            zy[num - 1] = (
                hcp[num - 1] / dt * t[num - 1]
                + fexp * tcp[num - 1] * (t[num - 2] - t[num - 1])
                + flx2
            )
            ...
            raise Exception(
                'Error during conduction calculation. Check input '
    ...
    def qsat(self, temp, pres, parameter):
        ...
        for i in range(len(temp)):
            ...
            # saturation humidity
            qsat_lst[i] = work2 * work1_lst[i] / (1.0 + ((work2 - 1.0) * work1_lst[i]))
