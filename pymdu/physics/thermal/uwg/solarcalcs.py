...
class SolarCalcs(object):
    ...
    def solarcalcs(self):
        ...
        * rural -- Rural road Element object
        ...
        if (self.dir + self.dif) > 0.0:
            ...
            self.Kw_term = min(
                abs(
                    1.0 / self.UCM.canAspect * (0.5 - self.critOrient / math.pi)
                    + 1 / math.pi * self.tanzen * (1 - math.cos(self.critOrient))
                ),
                1.0,
            )
            self.Kr_term = min(
                abs(
                    2.0 * self.critOrient / math.pi
            ...
            # in season
            if (
                self.simTime.month < self.parameter.vegStart
                or self.simTime.month > self.parameter.vegEnd
            ):
                alb_road = self.UCM.road.albedo
            else:
                alb_road = (
                    self.UCM.road.albedo * (1.0 - self.UCM.road.vegcoverage)
            ...
            fr = (
                1.0
                - (1.0 - 2.0 * self.UCM.wallConf) * self.UCM.alb_wall
                + (1.0 - self.UCM.roadConf)
                * self.UCM.wallConf
                * alb_road
                * self.UCM.alb_wall
            )
            ...
            self.mr = (
                ...
                + (1.0 - self.UCM.roadConf)
                * alb_road
                * (rw + self.UCM.wallConf * self.UCM.alb_wall * rr)
            ) / fr
            ...
            self.UCM.SolRecWall = (
                self.bldSol
                + (1 - 2 * self.UCM.wallConf) * self.UCM.road.albedo * self.roadSol
            )
            ...
            self.UCM.treeSensHeat = (
                (1 - self.parameter.vegAlbedo)
                * (1 - self.parameter.treeFLat)
                * self.UCM.SolRecRoad
                * self.UCM.treeCoverage
            )
            self.UCM.treeLatHeat = (
                (1 - self.parameter.vegAlbedo)
                * self.parameter.treeFLat
                * self.UCM.SolRecRoad
                * self.UCM.treeCoverage
            )
            ...
            self.UCM.treeSensHeat += (
                (1 - self.parameter.vegAlbedo)
                * (1 - self.parameter.grassFLat)
                * self.UCM.SolRecRoad
                * grasscover
            )
            self.UCM.treeLatHeat += (
                (1 - self.parameter.vegAlbedo)
                * self.parameter.grassFLat
                * self.UCM.SolRecRoad
                * grasscover
            )

        else:  # No Sun
            self.UCM.road.solRec = 0.0
            self.rural.solRec = 0.0

            for j in range(len(self.BEM)):
                self.BEM[j].roof.solRec = 0.0
                self.BEM[j].wall.solRec = 0.0

            self.UCM.SolRecRoad = 0.0  # Solar received by road
            self.UCM.SolRecRoof = 0.0  # Solar received by roof
            self.UCM.SolRecWall = 0.0  # Solar received by wall
            self.UCM.treeSensHeat = 0.0
            self.UCM.treeLatHeat = 0.0
    ...
    def solarangles(self):
        ...
        * RSM.GMT -- GMT hour correction
        ...
        self.ut = (24.0 + (int(secDay) / 3600.0 % 24.0)) % 24.0
        ...
        self.ad = 2.0 * math.pi / 365.0 * (date - 1 + (self.ut - (12 / 24.0)))

        self.eqtime = 229.18 * (
            0.000075
            + 0.001868 * math.cos(self.ad)
            - 0.032077 * math.sin(self.ad)
            - 0.01461 * math.cos(2 * self.ad)
            - 0.040849 * math.sin(2 * self.ad)
        )

        # Declination angle (angle of sun with equatorial plane)
        self.decsol = (
            0.006918
            - 0.399912 * math.cos(self.ad)
            + 0.070257 * math.sin(self.ad)
            - 0.006758 * math.cos(2.0 * self.ad)
            + 0.000907 * math.sin(2.0 * self.ad)
            - 0.002697 * math.cos(3.0 * self.ad)
            + 0.00148 * math.sin(3.0 * self.ad)
        )

        time_offset = self.eqtime - 4.0 * lon + 60 * GMT
        tst = secDay + time_offset * 60

        ha = (tst / 4.0 / 60.0 - 180.0) * math.pi / 180.0
        zlat = lat * (math.pi / 180.0)  # change angle units to radians

        # Calculate zenith solar angle
        self.zenith = math.acos(
            math.sin(zlat) * math.sin(self.decsol)
            + math.cos(zlat) * math.cos(self.decsol) * math.cos(ha)
        )

        # tangente of solar zenithal angle
        if abs(0.5 * math.pi - self.zenith) < 1e-6:
            if 0.5 * math.pi - self.zenith > 0.0:
                self.tanzen = math.tan(0.5 * math.pi - 1e-6)

            elif 0.5 * math.pi - self.zenith <= 0.0:
                self.tanzen = math.tan(0.5 * math.pi + 1e-6)
        ...
        self.critOrient = math.asin(min(abs(1.0 / self.tanzen) / canAspect, 1.0))
