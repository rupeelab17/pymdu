...
def urbflux(UCM, UBL, BEM, forc, parameter, simTime, RSM):
    ...
    UCM.roofTemp = 0.0  # Average urban roof temperature
    UCM.wallTemp = 0.0  # Average urban wall temperature
    ...
    for j in range(len(BEM)):
        ...
        BEM[j].roof.infra = e_roof * (forc.infra - sigma * T_roof**4.0)
        ...
        # exchange from road
        _infra_road_, BEM[j].wall.infra = infracalcs(
            UCM, forc, UCM.road.emissivity, e_wall, UCM.roadTemp, T_wall
        )

        # Update element temperatures
        BEM[j].mass.layerTemp = BEM[j].mass.Conduction(
            simTime.dt, BEM[j].building.fluxMass, 1.0, 0.0, BEM[j].building.fluxMass
        )
        BEM[j].roof.SurfFlux(
            forc,
            parameter,
    ...
    UCM.road.infra, _wall_infra = infracalcs(
        UCM, forc, UCM.road.emissivity, e_wall, UCM.roadTemp, UCM.wallTemp
    )
    UCM.road.SurfFlux(
        forc, parameter, simTime, UCM.canHum, T_can, UCM.canWind, 2.0, 0.0
    )
    ...
    try:
        UCM.latHeat += (
            UCM.latAnthrop + UCM.treeLatHeat + UCM.road.lat * (1.0 - UCM.bldDensity)
        )
    except TypeError:
    ...
    for iz in range(RSM.nzfor):
        ...
        y = (
            RSM.densityProfC[iz]
            * RSM.dz[iz]
            / (RSM.z[RSM.nzfor - 1] + RSM.dz[RSM.nzfor - 1] / 2.0)
        )
        t = forDens + y
        c1 += (t - forDens) - y
        forDens = t
    ...
    windUrb = (
        forc.wind
        * log(zref / RSM.z0r)
        / log(parameter.windHeight / RSM.z0r)
        * log(zrUrb / UCM.z0u)
        / log(zref / UCM.z0u)
    )
    dens = forc.pres / (1000 * 0.287042 * T_can * (1.0 + 1.607858 * UCM.canHum))

    # Friction velocity
    UCM.ustar = parameter.vk * windUrb / log((zrUrb - UCM.l_disp) / UCM.z0u)

    # Convective scaling velocity
    wstar = (parameter.g * max(UCM.sensHeat, 0.0) * zref / dens / Cp / T_can) ** (
        1 / 3.0
    )
    ...
    UCM.canWind = UCM.ustarMod * (UCM.verToHor / 8.0) ** (-1 / 2.0)
