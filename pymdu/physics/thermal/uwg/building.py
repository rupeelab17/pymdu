...
class Building(object):
    ...
    def __init__(
        ...
        self.msys = (
            coolcap
            / 1004.0
            / (min(self.cool_setpoint_day, self.cool_setpoint_night) - 14 - 273.15)
        )
    ...
    def condtype(self, value):
        ...
        assert value in (
            'AIR',
            'WATER',
        ), 'condtype must be "AIR" or "WATER". ' 'Got: {}.'.format(value)
    ...
    def from_dict(cls, data):
        ...
        assert data['type'] == 'Building', (
            'Expected ' 'Building dictionary. Got {}.'.format(data['type'])
        )
    ...
    def BEMCalc(self, UCM, BEM, forc, parameter, simTime):
        ...
        Qdehum = 0.0
        # dens: Moist air density given dry bulb example, humidity ratio, and pressure
        dens = moist_air_density(forc.pres, self.indoor_temp, self.indoor_hum)  # kgv/m3
        evapEff = 1.0  # evaporation efficiency in the condenser
        # total vent volumetric flow [m3 s-1]
        volVent = self.vent * self.nFloor
        volInfil = (
            self.infil * UCM.bldHeight / 3600.0
        )  # Change of units AC/H -> [m3 s-1]
        T_wall = BEM.wall.layerTemp[-1]  # Inner layer
        volSWH = BEM.swh * self.nFloor / 3600.0  # Change of units l/hr per m^2 -> [L/s]
        T_ceil = BEM.roof.layerTemp[-1]  # Inner layer
        T_mass = BEM.mass.layerTemp[0]  # Outer layer
        T_indoor = self.indoor_temp  # Indoor example (initial)
        ...
        wallArea = facArea * (1.0 - self.glazing_ratio)  # [m2(wall)/m2(bld)]
        ...
        # schedule & simTime hr
        isEqualNightStart = is_near_zero(
            (simTime.secDay / 3600.0) - parameter.nightSetStart
        )
        if (simTime.secDay / 3600.0 < parameter.nightSetEnd) or (
            simTime.secDay / 3600.0 > parameter.nightSetStart or isEqualNightStart
        ):
        ...
        self.sensCoolDemand = max(
            ...
            +
            # window load due to example delta
        ...
        if self.sensCoolDemand > 0.0 and UCM.canTemp > 288.0:
            ...
            self.dehumDemand = max(
                VolCool * dens * (self.indoor_hum - 0.9 * 0.0078) * parameter.lv, 0.0
            )
            ...
            self.coolConsump = (
                max(self.sensCoolDemand + self.dehumDemand, 0.0)
            ) / self.cop_adj
            ...
            if self.condtype == 'AIR':
                ...
                self.sensWaste = max(
                    self.sensCoolDemand + self.dehumDemand, 0
        ...
        # Assumes air example of control volume is sum of surface boundary temps
        # weighted by area and heat transfer coefficient + generated heat
        self.indoor_temp = (H1 + Q) / H2

        self.indoor_hum = self.indoor_hum + (
            simTime.dt / (dens * parameter.lv * UCM.bldHeight)
        ) * (QLintload + QLinfil + QLvent - Qdehum)

        # Calculate relative hum (Pw/Pws*100) using pressurce, indoor example, hum
        _Tdb, _w, _phi, _h, _Tdp, _v = psychrometrics(
            self.indoor_temp, self.indoor_hum, forc.pres
        )
        ...
        self.fluxMass = (
            zac_in_mass * (T_indoor - T_mass)
            + self.int_heat * self.int_heat_f_rad / massArea
        )
        ...
        CpH20 = 4200.0  # specific heat capacity of water J/(kg-C)
        T_hot = 49 + 273.15  # Service water example (assume no storage)
        # N.B No L to kg conversion for water in this equation because
        # 1 L of water = 1 kg of water.
        self.sensWaste = (
            self.sensWaste
            + (1 / self.heateff - 1.0) * (volSWH * CpH20 * (T_hot - forc.waterTemp))
            + BEM.gas * (1 - self.heateff) * self.nFloor
        )

        # Gas equip per floor + water usage per floor + heating/floor
        self.GasTotal = (
            BEM.gas
            + volSWH * CpH20 * (T_hot - forc.waterTemp) / self.nFloor / self.heateff
            + self.heatConsump
        )
    ...
    def __repr__(self):
        return (
            ...
            'initial_temp: {}'.format(
                ...
                self.condtype,
                self.cop,
                self.cool_setpoint_day,
                self.cool_setpoint_night,
                self.heat_setpoint_day,
                self.heat_setpoint_night,
                self.coolcap,
                self.heateff,
                self.initial_temp,
            )
