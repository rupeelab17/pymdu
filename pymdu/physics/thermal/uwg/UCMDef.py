...
class UCMDef(object):
    ...
    def __init__(
        ...
        self.wallConf = (
            0.5
            * (self.canAspect + 1 - pow(pow(self.canAspect, 2) + 1, 0.5))
            / self.canAspect
        )
        ...
        frontDens = verToHor / 4.0  # density of just street facing facade
        if frontDens < 0.15:
    ...
    def UCModel(self, BEM, T_ubl, forc, parameter):
        ...
        # air density
        dens = forc.pres / (
            1000 * 0.287042 * self.canTemp * (1.0 + 1.607858 * self.canHum)
        )
        dens_ubl = forc.pres / (1000 * 0.287042 * T_ubl * (1.0 + 1.607858 * forc.hum))
        Cp_air = parameter.cp
        ...
        H1 = (
            H1 + T_ubl * self.roadArea * self.uExch * Cp_air * dens_ubl
        )  # Heat from UBL
        ...
        for j in range(len(BEM)):
            ...
            A_wall = (1.0 - R_glazing) * self.facArea
            ...
            self.Q_ubl = self.Q_ubl + BEM[j].frac * self.bldDensity * (
                BEM[j].roof.sens + BEM[j].building.sensWaste * (1.0 - self.h_mix)
            )  # Changed by Jiachen Mao in March 2017
        ...
        self.Q_road = h_conv * (T_road - self.canTemp) * (1.0 - self.bldDensity)
        self.Q_ubl = self.Q_ubl + self.uExch * Cp_air * dens * (
            self.canTemp - T_ubl
        ) * (1.0 - self.bldDensity)
        ...
        for j in range(len(BEM)):
            ...
            self.Q_window = self.Q_window + BEM[
                j
            ].frac * self.verToHor * R_glazing * U_window * (T_indoor - T_can)
            self.Q_window = self.Q_window + BEM[
                j
            ].frac * self.verToHor * R_glazing * BEM[j].wall.solRec * (
                1.0 - BEM[j].building.shgc
            )
            self.Q_vent = self.Q_vent + BEM[
                j
            ].frac * self.bldDensity * Cp_air * dens * (V_vent + V_infil) * (
                T_indoor - T_can
            )
            self.Q_hvac = (
                self.Q_hvac
                + BEM[j].frac * self.bldDensity * BEM[j].building.sensWaste * self.h_mix
            )
            ...
            self.ElecTotal = (
                self.ElecTotal + BEM[j].fl_area * BEM[j].building.ElecTotal / 1.0e6
            )
            self.GasTotal = (
                self.GasTotal + BEM[j].fl_area * BEM[j].building.GasTotal / 1.0e6
            )
        ...
        # Error checking
        if self.canTemp > 350.0 or self.canTemp < 250:
            raise Exception(
                'Got canyon temperature at {} C. Something obviously went '
