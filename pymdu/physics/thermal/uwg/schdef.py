...
class SchDef(object):
    ...
    def from_dict(cls, data):
        ...
        """
        assert data['type'] == 'SchDef', (
            'Expected ' 'SchDef dictionary. Got {}.'.format(data['type'])
        )
    ...
    def check_week_validity(week, name):
        assert isinstance(week, (list, tuple)), (
            'The {} property must be a ' 'list or tuple. Got {}.'.format(name, week)
        )

        assert len(week) == 3, (
            'The {} property must be a 3 x 24 matrix. Got ' '{} rows.'.format(
                name, len(week)
            )
        )
