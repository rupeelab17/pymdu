...
class UBLDef(object):
    ...
    def __init__(
        self, location, charLength, initialTemp, maxdx, dayBLHeight, nightBLHeight
    ):
        self.location = location
        self.charLength = charLength
        self.perimeter = 4.0 * charLength
    ...
    def ublmodel(self, UCM, RSM, rural, forc, parameter, simTime):
        ...
        for iz in range(RSM.nzref):
            refDens = refDens + RSM.densityProfC[iz] * RSM.dz[iz] / (
                RSM.z[RSM.nzref - 1] + RSM.dz[RSM.nzref - 1] / 2.0
            )
        ...
        is_day = (
            (sunlight > daylimit)
            and (time < noon or is_near_zero(time - noon))
            or (sunlight > nightlimit)
            and (time > noon)
            or (self.sensHeat > 150.0)
        )
        if is_day:
        ...
        # ---------------------------------------------------------------------
            ...
            Csurf = UCM.Q_ubl * simTime.dt / (h_UBL * refDens * Cp)
            self.ublTemp, self.ublTempdx = UBLDef.nightforc(
                self.ublTempdx,
                simTime.dt,
                h_UBL,
                self.paralLength,
