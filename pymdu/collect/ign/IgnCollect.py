...
class IgnCollect(GeoCore):
    ...
    _cql_filter: str = None
    _cutline_from_polygon: Polygon = None
    ...
    def __init__(self):
        ...
        path_filename_ressource = str(
            self.collect_path.joinpath('ign/Tableau-suivi-services-web-11-10-2024.csv')
        )
    ...
    def execute_ign(self, key: str = 'buildings', **kwargs):
        ...
        if key in [
            ...
            if self._cql_filter:
                ...
                # self.filterxml = ElementTree.tostring(filter.toXML()).decode("utf-8")

                BBOX = BBox(bbox=self._bbox, crs='EPSG:4326')
                print(BBOX)
                self.filterxml = ElementTree.tostring(
                    BBOX.toXML(), encoding='ascii', method='xml', xml_declaration=True
                ).decode('utf-8')
                print(self.filterxml)

                # filter1 = fes2.PropertyIsEqualTo("nom_com", "Poitiers")
                # fr = fes2.FilterRequest()
                # self.filterxml = fr.setConstraint(filter1, tostring=True)
                # print(self.filterxml)
                # self.filterxml = "<Filter><BBOX><PropertyName>Geometry</PropertyName> <Box srsName='EPSG:4326'><coordinates>%f,%f %f,%f</coordinates> </Box></BBOX></Filter>" % (
            ...
            wms = WebMapService(url=url, version='1.3.0', timeout=130)
            # GetMap (image/jpeg)
